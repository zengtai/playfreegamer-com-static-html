import { getGameDataById, getAllGamesWithId } from "@/lib/api";
      ...
      <div className="container">
        <div className="m-4 flex">
          ...
          <div className="meta ml-3">
            <h1 className="mb-2 text-xl font-black leading-5 text-rose-500">
              {i.title}
            </h1>
        ...
        <div className="mx-4 flex justify-center">
          <button className="w-full rounded-full border-2 border-rose-200 bg-rose-100 to-white px-4 py-2 text-sm font-bold text-rose-500 shadow shadow-rose-50">
            Play Now
          </button>
        </div>
        <div className="m-4 text-sm text-gray-500">{i.description}</div>
        <div className="m-4">Related Games</div>
...
export const getStaticProps = async (ctx) => {
  const data = await getGameDataById({ id: ctx.params.id });
  return {
    props: {
      data,
    },
...
export const getStaticPaths = async () => {
  const games = await getAllGamesWithId();
  const paths = games.map((i) => ({ params: { id: i.appid } }));
  return {
    paths,
    fallback: false,
  };
